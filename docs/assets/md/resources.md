## Resource

### NoTitle

<p align="center">
  <img src="assets/grepiResourceLogoPic.png" style="width:92%;margin: auto;" align = "center">
</p> 

<br>
<p style="padding: 2rem 1.75rem;"><a href="https://who-collaboratory.github.io/collaboratory-grepi-web/" target="_blank" rel="noopener noreferrer">grEPI</a> is a living global repository of epidemiological parameters hosted by the World Health Organization.</p>

### NoTitle

<p align="center">
  <img src="assets/epireviewResourceLogoPic.png" style="width:100%;margin: auto;" align = "center">
</p> 

<br>
<p style="padding: 2rem 1.75rem;"><a href="https://mrc-ide.github.io/epireview/" target="_blank" rel="noopener noreferrer">epireview</a> is a tool to obtain the latest data, figures and tables from the <a target="_blank"  href="https://www.imperial.ac.uk/mrc-global-infectious-disease-analysis/related-initiatives/perg/">Pathogen Epidemiology Review Group (PERG)</a>.This package also contains functions to update pathogen-specific databases with new data from peer-reviewed papers as they become available.</p>

### NoTitle

<p align="center">
  <img src="assets/epiparameterResourceLogoPic.png" style="width:92%;margin: auto;" align = "center">
</p> 

<br>
<p style="padding: 2rem 1.75rem;"><a href="https://epiverse-trace.github.io/epiparameter/" target="_blank" rel="noopener noreferrer">epiparameter</a>  is an R package, developed at the <a target="_blank" href="https://www.lshtm.ac.uk/research/centres/centre-mathematical-modelling-infectious-diseases">Centre for the Mathematical Modelling of Infectious Diseases</a> at the <a target="_blank" href="https://www.lshtm.ac.uk/">London School of Hygiene and Tropical Medicine</a> as part of <a target="_blank" href="https://data.org/our-work/epiverse/">Epiverse-TRACE</a>, that contains a library of epidemiological parameters for infectious diseases as well as classes and helper functions to work with the data.</p>

### <a href="https://doi.org/10.17605/OSF.IO/7K4GU" target="_blank" rel="noopener noreferrer">Systematic review of epidemiological parameters for measles</a> 
<br>Public Health Agency of Canada (PHAC)


### <a href="https://www.crd.york.ac.uk/PROSPERO/view/393345" target="_blank" rel="noopener noreferrer">Systematic review of transmission and modelling parameters of nine WHO blue-print priority pathogens</a> 
Imperial College London - Pathogen Epidemiology Review Group (PERG)


### <a href="https://worldhealthorg.shinyapps.io/mpx_global/#sec-lit" target="_blank" rel="noopener noreferrer">Highlight studies that estimate key epidemiological parameters for mpox</a>
<br>Public Health Agency of Canada (PHAC) and World Health Organization (WHO)
